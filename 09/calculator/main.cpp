...
int main() {
    ...
    while ( true ) {
        ...
        transform(command_to_be_executed.begin(), command_to_be_executed.end(),
                  command_to_be_executed.begin(),::toupper);

        // Find the command in COMMANDS
        auto it = find_if(COMMANDS.begin(), COMMANDS.end(),
            [&](const Command& cmd) { return cmd.str == command_to_be_executed; });
        if (it != COMMANDS.end()) {
            // Command found
            const Command& cmd = *it;

            // Check if the correct number of parameters is provided
            if (pieces.size() - 1 != cmd.parameter_number) {
                cout << "Error: wrong number of parameters." << endl;
            } else if (pieces.size() == 1 && cmd.is_exit) { // Check if it's an exit command
                    cout << GREETING_AT_END << endl;
                    break;
            } else {
                // Convert parameters to double using string_to_double
                double param1, param2;
                if (string_to_double(pieces[1], param1) && string_to_double(pieces[2], param2)) {
                    // Execute the command
                    double result = cmd.action(param1, param2);
                    cout << result << endl;
                } else {
