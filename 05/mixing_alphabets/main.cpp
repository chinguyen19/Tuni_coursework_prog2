...
int main()
{
    ...
    while (std::cin >> word)
    {
        ...
        if (word.size()==1) {
            ...
            std::string::iterator letter_begin = next(word.begin(),1);
            std::string::iterator letter_end = next(word.begin(), word.size()-1);
            std::shuffle(letter_begin, letter_end, generator);
            std::cout << word << std::endl;
        }
    }
}
