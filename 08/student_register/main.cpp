...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string studentNumberToChange = parts[1];

                            auto it = student_numbers.find(studentNumberToChange);
                            ...
                            if (!is_valid_phone_number(newPhoneNumber)) {
                                continue; // Invalid phone number
                            }

                            // Update phone number in the data structure
                            student->phone_number = newPhoneNumber;

                            // Update phone number in the file
                            std::ofstream file(file_name);
                            if (file.is_open()) {
                                for (const auto& pair : student_numbers) {
                                    const Student& currentStudent = *(pair.second);
                                    file << currentStudent.student_number << ';' << currentStudent.user_id << ';' << currentStudent.name << ';'
                                         << currentStudent.phone_number << ';' << currentStudent.email << ';' << currentStudent.skype << '\n';
                                }
                                file.close();
                            }
